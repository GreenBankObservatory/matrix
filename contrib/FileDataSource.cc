/*******************************************************************
 *  FileDataSource.cc - Implementation of a Component which
...
void FileDataSource::_reader_thread()
{
    FILE *fin;
...
bool FileDataSource::disconnect()
{
    // Source only Components really don't need connect/disconnect
    return true;
}

bool FileDataSource::_stop()
{
...
bool FileDataSource::_do_start()
{
    ...
    {
        cout << "FileDataSource::_do_start(): starting thread." << endl;
        _run.set_value(true);
    ...
    if (rval)
    {
        cout << "FileDataSource started." << endl;
    }
    else
    {
        cout << "FileDataSource failed to start!" << endl;
...
bool FileDataSource::_do_stop()
{
    if (_read_thread.running())
    {
        cout << "FileDataSource::_do_stop(): Killing thread." << endl;
        _run.signal(false);
        _read_thread.join();
        cout << "FileDataSource dead." << endl;
    }
